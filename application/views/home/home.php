            ...
            <div class="col-lg-12 col-md-12">
              ...
              <div id="new-updates" class="card updates recent-updated">
                <div id="updates-header" class="card-header d-flex justify-content-between align-items-center">
                  <h2 class="h5 display"><a data-toggle="collapse" data-parent="#new-updates" href="#map-box" aria-expanded="true" aria-controls="map-box">Maps</a></h2><a data-toggle="collapse" data-parent="#new-updates" href="#map-box" aria-expanded="true" aria-controls="map-box"><i class="fa fa-angle-down"></i></a>
                </div>
                <div id="map-box" role="tabpanel" class="collapse show">
...
<link href="<?php echo base_url('assets/bootstrapdashboard/css/mapbox.css')?>" rel='stylesheet' />
<script src="<?php echo base_url('assets/bootstrapdashboard/js/mapbox.js')?>"></script>
<script type="text/javascript">
  ...
  {   
      ...
      $.ajax({
        type: "POST",
        url: "<?php echo base_url('t_disasteroccur/getalldata')?>",
  ...
  function loadMapBox(data){
    if (data != undefined){
      L.mapbox.accessToken = 'pk.eyJ1IjoiYW5kaWthcnlhbnRvbyIsImEiOiJjamtidnRnNXEycXAwM3FuY2xneXBiZjN3In0.kh8f-Obs2m7H01Gr_djicw';
      var map = L.mapbox.map('map', 'mapbox.streets') 
      .setView([data[0].Latitude, data[0].Longitude], 13);

      //add style
      //var styleLayer = L.mapbox.styleLayer('mapbox://styles/mapbox/light-v9').addTo(map);

      //add marker here
      for(i=0; i<data.length;i++){
        var logistic = data[i].IsNeedLogistic == 1 ? "Ya" : "Tidak"; 
        ...
        L.marker([data[i].Latitude, data[i].Longitude], {
          ...
          title : data[i].DisasterName
        })
        .addTo(map)
        .bindPopup(
              '<div class="" >' +
              '<h6 style="color:black;font-weight:bold">'+ "Bencana : "+ data[i].DisasterName +'</h6>' +
              '<p style="color:black">'+  "NoDisaster : "+data[i].NoDisaster+ '<br>'+
                                          "Kelurahan : " +data[i].VillageName+'<br>' + 
                                          "RT : " +data[i].RT+'<br>' +
                                          "RW : " +data[i].RW+'<br>' + 
                                          "Longitude : " +data[i].Longitude+'<br>' + 
                                          "Latitude : " +data[i].Latitude+'<br>' + 
                                          "Deskripsi : " +data[i].Description+'<br>' + 
                                          "Butuh Logistik? : " +logistic+'<br>' + 
              '</p>'+
              '</div>');
      }
    }
  ...
  function loadGoogleMap(data){
    ...
    if (data != undefined){
        ...
        for(i=0; i<data.length;i++)
        {
            markers[i] = [data[i].NoDisaster,data[i].Latitude, data[i].Longitude, null, null];
            var logistic = data[i].IsNeedLogistic == 1 ? "Ya" : "Tidak"; 
            infoWindowContent [i] = ['<div class="" >' +
            '<h6 style="color:black;font-weight:bold">'+ "Bencana : "+ data[i].DisasterName +'</h6>' +
            '<p style="color:black">'+"NoDisaster : "+data[i].NoDisaster+ '<br>'+
                                        "Kelurahan : " +data[i].VillageName+'<br>' + 
                                        "RT : " +data[i].RT+'<br>' +
        ...
        for (i = 0; i < markers.length; i++) {
            ...
            var icon = "<?php echo base_url('assets/resources/img/red-dot.png');?>";
            if(markers[i][4] != null)
            {
                icon = markers[i][4];
            }
      ...
      {
      var mapProp= {
          center:new google.maps.LatLng(-7.803249,110.3398254),
          zoom:5
          };
...
<script type="text/javascript" src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDCYfMD5ZhTfqN-Arj8FBas1-ciLbOdUBY&callback=myMap"></script>
